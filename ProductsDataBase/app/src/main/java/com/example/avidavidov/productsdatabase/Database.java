...
public class Database {
    ...
    public static abstract class Table {
        ...
        public static String getDrop() {
            return "DROP TABLE ID EXITS " + name;
        }
    ...
    private static boolean isWriteable = false;
    ...
    public static long addProduct (String porductName, int categoryId, float unitPrice, int unitInStock, boolean isContinued){
        if (helper==null || db == null || isWriteable==false)
            throw new IllegalStateException();
        ContentValues contentValues = new ContentValues();
        contentValues.put (TableProducts.columns[1].name,porductName);
        contentValues.put (TableProducts.columns[2].name,categoryId);
        contentValues.put (TableProducts.columns[3].name,unitPrice);
        contentValues.put (TableProducts.columns[4].name,unitInStock);
        contentValues.put (TableProducts.columns[5].name,isContinued?1:0);
    ...
    public static boolean deleteProduct (long productId){
        ...
        return db.delete(TableProducts.name, TableProducts.columns[0].name + "=" + productId, null) >0;
    }

    public static boolean updateProduct (long productId, String productName, int categoryId, float unitPrice, int unitInStock, boolean isContinued) {
        if (helper == null || db == null || isWriteable == false)
            throw new IllegalStateException();
        ContentValues contentValues = new ContentValues();
        ...
        contentValues.put (TableProducts.columns[4].name,unitInStock);
        contentValues.put (TableProducts.columns[5].name,isContinued?1:0);
        return db.update(TableProducts.name, contentValues, TableProducts.columns[0].name + "=" + productId,null) >0;
    }
